...
beta_s = ''.join(str(beta).split('.'))
gamma_s = ''.join(str(gamma).split('.'))
lambda_s = ''.join(str(lambda_param).split('.'))

# ../saved/log_b02l09re
# ../saved/log_b05l07re_23
if saved_log_path != './log':
    _saved_log_path = saved_log_path.split('/')[-1].split('_')
    if _saved_log_path[-1] == '23':
        num_classes = 23
        _saved_log_path = _saved_log_path[-2]
    else:
        _saved_log_path = _saved_log_path[-1]
    param, types = _saved_log_path[:-2], ('real' if _saved_log_path[-2:] == 're' else 'semantic_annotations')
    params = param.split('l') 
    if len(params) == 2:
        lambda_s = params[1]
        beta_s = params[0][1:]
    elif len(params) == 1:
...
if lambda_s[0] == '0':
    lambda_param = float('0.' + lambda_s[1])
else:
    lambda_param = int(lambda_s)
...
## jideca_b01r01re_34
data_name = "jideca_b" + beta_s + "g" + gamma_s + "l" + lambda_s + ("re" if types == "real" else "se") + "_" + str(num_classes)
